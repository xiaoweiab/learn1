...
# inputs 是一个形如（batch_size,seq_len,word_size）的张量
# 函数返回一个形如（batch_size,seq_len,word_position）的位置张量
...
'''
inputs 是一个二阶以上的张量，代表输入序列，比如形如（batch_size,seq_len,input_size）
seq_len 是一个形如（batch_size,）的张量，代表每个序列的实际长度，多出来的部分都会被忽略
mode分为mul 和 add , mul 是指把多出的部分全部至零，一般用于全连接层；
...
'''
普通的全连接
inputs 是一个二阶或二阶以上的张量，即如（batch_size,...,input_size）
...
def Dense(inputs, output_size, bias=True, seq_len=None):
    ...
    outputs = tf.reshape(outputs, tf.concat(
        [tf.shape(inputs)[:-1], [output_size], 0]))
    if seq_len != None:
...
def Attention(Q,K,V,nb_head,size_per_head,Q_len=None,V_len=None):
    ...
    K = tf.shape(K,(-1,tf.shape(K)[1],nb_head,size_per_head))
    K = tf.transpose(K,[0,2,1,3])

    V = Dense(V,nb_head*size_per_head,False)
    V = tf.shape(V,(-1,tf.shape(V)[1],nb_head,size_per_head))
    V = tf.transpose(V,[0,2,1,3])

    # 计算内积，然后mask 再softmax
